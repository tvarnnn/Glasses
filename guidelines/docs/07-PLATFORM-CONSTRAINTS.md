...
**2026-08-20/21 status update — Tower half done, iOS half outstanding.** The Tower now *accepts* optional `source_seq`/`tx_seq` fields and reports `tx_seq_gap_total` (commit `0346c05`), but the iOS sender does not yet send them, so `tx_seq_gap_total` still reads `null` and **genuine transit loss remains formally unmeasured on this platform**. See `docs/superpowers/handoffs/2026-08-20-source-seq-tx-seq-split.md`.

Separately, the Tower can now estimate the two quantities it *can* derive without any protocol change: `sampling_stride_avg` (capture frames elapsed per frame received) and `source_fps_estimate` (the upstream capture rate being sampled), both computed from `source_seq` with its fallback to `seq`. These do not distinguish sampling from loss — only the `tx_seq` split can do that — but they do remove the need to infer the *sender's throttle* by hand from `seq_gap_total`. Added 2026-08-21 after the first physical-glasses remote run (`guidelines/docs/reports/2026-08-21-first-physical-glasses-remote-baseline.md`) required exactly that arithmetic.

**2026-08-21 finding — the 1-in-30 stride is the platform's observation-rate bottleneck, and it is rate-independent of the transport.** The first physical-glasses run delivered 0.8 fps over a remote Tailscale path; the 2026-08-19 LAN soak delivered 0.81 fps. A fixed decimation stride bounds delivered rate to capture-rate ÷ stride regardless of available bandwidth, so improving the link cannot improve the observation rate. Note also that the "1-in-30" mechanism itself has still never been read off the Swift source (Rule 4) — the wire arithmetic confirms the *effective* stride is ~30 but not whether it is implemented as a frame-count stride or a timer. Fix and verification are specified in `docs/superpowers/handoffs/2026-08-21-ios-observation-rate.md`.
