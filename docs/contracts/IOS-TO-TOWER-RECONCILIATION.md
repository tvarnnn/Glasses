...
| 4.2 | Orientation `unknown`/`towardCamera`/`awayFromCamera`/`acrossView` | **BLOCKED (transport)**, and **off by default** | Implemented from COCO eye/ear keypoint **visibility**, which is genuine coarse evidence. It costs a measured **798 ms** per call on this CPU — 24× the detector and 2.5× the delivered frame interval — so it runs at a bounded cadence and every estimate carries its age. The unblocker is a CUDA build, not a different algorithm |
