...
dependency study exonerated it, and the install confirms that
exoneration:
...
dependency, un-gated tests that had been silently skipping:
`tests/test_world_builder_redaction.py` and one case in
`tests/test_world_builder_pose_accuracy.py` both `importorskip
("skimage.data")` for sample faces. On the tree as it stood when this
sweep began, installing the `[ocr]` extra moved the suite from 1307
passed / 32 skipped to **1319 passed / 30 skipped** — 12 World Builder
tests that had never run, all passing. Those 38 tests were re-run
directly to confirm: **38 passed**.
...
Six. OCR was then run on all six warped crops:
...
The branch advanced underneath this work — World Builder registration
commits landed mid-sweep — so the suite figure quoted in §0 (1307→1319)
is from the tree the sweep started on. On the tree this report is
committed to, the full suite reads **1362 passed, 30 skipped, 0 failed**
(283 s), plus **14 passed** in the opt-in `TOWER_RUN_MODEL_TESTS=1`
real-model suite. No test fails in either state.
